...
(function (global, factory) {
	...
	var dropdownCheck = renderer.create(
		'<div class="dropdown-menu note-check" role="list">',
		function ($node, options) {
			var markup = $$1.isArray(options.items)
				? options.items
						.map(function (item) {
			...
			$node.html(
				[
					'<label class="form-check-label"' +
						...
						'>',
					' <input role="checkbox" type="checkbox" class="form-check-input"' +
	...
	var ui = {
		...
		button: function ($node, options) {
			return renderer.create(
				'<button type="button" class="note-btn btn btn-light btn-sm" role="button" tabindex="-1">',
				function ($node, options) {
